...
const EstimateSmallItem: React.FC<EstimateProps> = ({ projectId, user }) => {
    ...
    const getDataPosition = async (data: Position) => {
     const newData =  await addLowPosition({ projectId, estimateId: estId, title: data.title,
  ...
  setData(prevData => {
    ...
    setData(prevData => {
       ...
       const handleExcelGeneration = () => {
            if (project && data) {
                generateAndDownloadExcel(project, data, "small");
            } else {
            ...
            <table className="bg-white rounded-lg shadow-pricesTablet">
                         ...
                         <td className="border border-gray-20 p-3 flex items-center justify-center gap-6">
                            <button
                                ...
                                addIsToggle(position.id, !position.isShow, "update", "position");
                                    if (position.isShow) {
                                     const data = await updateLowPosition({
                                            projectId: projectId,
                                            estimateId: item.id,
                ...
                <div>
                    ...
                    <div className="flex items-center justify-between mb-8">
                        ...
                        <p className="text-lg font-normal">{project?.lowTotal && roundingNumber(project?.lowTotal)}</p>
                    </div>  
                    ...
                    {isShowTotals && (
                     ...
                     <div className="flex items-center justify-between p-6 bg-gray-5 rounded-full">
                        ...
                        <p className="text-xl font-semibold">{project?.lowGeneral && roundingNumber(project?.lowGeneral)}</p>
                    </div>
                        </>   
                    )}
