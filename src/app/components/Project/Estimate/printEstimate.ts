...
export const handlePrint = (data: ProjectItem | null) => {
  const printContent = `
    <div class="print-content">
       <h3 class="title">Кошторис</h3>

    ${data && data.estimates?.map((estimate) => `
    ...
    `).join('')}
       ...
       <div class="w-5/6 flex items-center justify-between mb-8 mt-8">
           ...
           <p class="text-lg font-normal">${data?.total && roundingNumber(data?.total)}</p>
       </div>

        ${data?.discount !== 0 && `
          <div class="w-5/6 flex items-center justify-between mb-4">
              <p class="text-lg font-normal text-green">Знижка:</p>
              <p class="text-lg font-normal text-green">${data?.discount && roundingNumber(data?.discount)}</p>
          </div>
        `}
      <div class="w-5/6 flex items-center justify-between mb-4">
      ...
      <div class="w-5/6 flex items-center justify-between mb-4">
       ...
       <div class="w-5/6 flex items-center justify-between p-6 bg-gray-5 rounded-full">
            ...
            <p class="text-xl font-semibold">${data?.general && roundingNumber(data?.general)}</p>
       </div>
