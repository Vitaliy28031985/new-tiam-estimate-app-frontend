...
const SendLowEstimatePdf: React.FC<SendEstimatePdfProps> = ({ data }) => {
  ...
  const generatePrintContent = () => {
    ...
    return (
      <>
        {data.lowEstimates?.map((estimate, estimateIndex) => (
      ...
      if (navigator.share) {
        try {
          await navigator.share({
            files: [new File([pdfBlob], 'estimate-low.pdf', { type: 'application/pdf' })],
            title: 'Кошторис',
...
export default SendLowEstimatePdf;
